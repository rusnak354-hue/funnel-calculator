...
export default function FunnelCalculator() {
  ...
  const loadFromURL = () => {
    ...
    if (sharedData) {
      try {
        // Декодуємо дані з URL (з підтримкою Unicode)
        const decoded = decodeURIComponent(sharedData);
        const binaryString = atob(decoded);
        // Конвертуємо binary string назад в UTF-8
        const jsonString = decodeURIComponent(
          binaryString.split('').map(c => 
            '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)
          ).join('')
        );
        const dashboardData: DashboardData = JSON.parse(jsonString);
        
        // Завантажуємо дані в калькулятор
        setDashboardName(dashboardData.name + ' (копія)');
        setImpressions(dashboardData.impressions);
        setStages(dashboardData.stages);
        setMetrics(dashboardData.metrics);
        setCurrentDashboard(null); // Це новий дашборд для користувача
      } catch (error) {
        console.error('Error loading shared dashboard:', error);
        // Якщо помилка - показуємо дефолтні дані
      }
    }
  };
  ...
  const createShareLink = () => {
    ...
    try {
      // Кодуємо дані в base64 з підтримкою Unicode
      ...
      const link = `${baseUrl}?data=${urlEncoded}`;
  ...
  const clearAll = () => {
    // Для нових дашбордів - всі конверсії = 0 (будуть автоматично розраховуватись)
    setStages([
  ...
  const updateStageValue = (stageId: string, field: keyof Stage, value: string) => {
    ...
    const updatedStages = stages.map(s => 
      s.id === stageId ? { ...s, [field]: numValue } : s
    );
    
    // Якщо змінили value (кількість), автоматично перераховуємо конверсії
    if (field === 'value') {
      const stageIndex = updatedStages.findIndex(s => s.id === stageId);
      
      if (stageIndex === 0) {
        // Перший етап (Кліки) - конверсія від показів
        if (impressions > 0) {
          updatedStages[stageIndex].conversion = Math.round((numValue / impressions) * 10000) / 100;
        }
      } else if (stageIndex > 0) {
        // Інші етапи - конверсія від попереднього етапу
        const prevValue = updatedStages[stageIndex - 1].value;
        if (prevValue > 0) {
          updatedStages[stageIndex].conversion = Math.round((numValue / prevValue) * 10000) / 100;
        }
      }
    }
  ...
  const updateImpressions = (value: string) => {
    const numValue = parseFloat(value) || 0;
    setImpressions(numValue);
    
    // Автоматично перераховуємо конверсію для першого етапу (Кліки)
    if (numValue > 0 && stages[0]) {
      const clicksValue = stages[0].value;
      const newConversion = Math.round((clicksValue / numValue) * 10000) / 100;
      
      setStages(stages.map((s, i) => 
        i === 0 ? { ...s, conversion: newConversion } : s
      ));
    }
  };
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-slate-600 text-xs font-semibold block mb-1.5">
                          Конверсія (%) 
                          <span className="text-slate-400 ml-1">автоматично</span>
                        </label>
                        <input
                          type="number"
                          step="0.01"
                          ...
                          readOnly
                          placeholder="0"
                          className="w-full px-3 py-2 bg-slate-50 border-2 border-slate-200 rounded-xl text-slate-600 text-sm text-center cursor-not-allowed"
                          title="Конверсія розраховується автоматично"
                        />
                      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8 bg-gradient-to-br from-indigo-50 to-purple-50 p-6 rounded-2xl border-2 border-indigo-100/60">
              <div>
                ...
                <input
                  type="number"
                  value={impressions || ''}
                  onChange={(e) => updateImpressions(e.target.value)}
                  placeholder="0"
