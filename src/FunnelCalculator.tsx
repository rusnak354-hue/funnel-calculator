...
export default function FunnelCalculator() {
  ...
  const loadFromURL = () => {
    ...
    if (sharedData) {
      try {
        // Декодуємо дані з URL
        const decoded = decodeURIComponent(sharedData);
        const dashboardData: DashboardData = JSON.parse(atob(decoded));
  ...
  const createShareLink = () => {
    ...
    try {
      // Кодуємо дані в base64 та додаємо в URL
      const encoded = btoa(JSON.stringify(dashboardData));
