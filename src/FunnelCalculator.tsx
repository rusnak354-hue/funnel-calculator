...
export default function FunnelCalculator() {
  ...
  const calculateForecast = () => {
    ...
    // Перевіряємо чи є взагалі зміни
    const hasAnyGrowth = stages.some(s => s.growth && s.growth !== 0);
    ...
    for (let i = 1; i < forecast.length; i++) {
      if (i === 1) {
        ...
        const leadGrowthFactor = forecast[i].growth / 100; // Власна зміна лідів
        const baseLeadCrWithGrowth = baseLeadCr * (1 + leadGrowthFactor);
        
        // Застосовуємо decay тільки якщо є ріст CTR
        if (decayResult.growthFactor > 0) {
          // Є ріст CTR → застосовуємо decay до базової конверсії + зміна
          forecast[i].newConversion = decayResult.newLeadCr * (1 + leadGrowthFactor);
        } else {
          // Немає росту CTR → просто базова конверсія + зміна
